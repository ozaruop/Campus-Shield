...
async function studentLogin() {
  ...
  try {
    ...
    // Save session immediately — don't let Firestore write block login
    saveSession(studentData);

    // Write to Firestore in background (non-blocking)
    if (window.db) {
      window.db.collection("students").doc(id).set(studentData, { merge: true })
        .catch(err => console.warn("Student profile save failed (non-critical):", err));
    }
